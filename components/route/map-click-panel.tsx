...
import { X, Plane, MapPin, Mountain, Crosshair, PlaneTakeoff, PlaneLanding, Radio, Triangle, Undo } from 'lucide-react';
...
export function MapClickPanel({ clickedPoint, onClose }: MapClickPanelProps) {
  ...
  useEffect(() => {
    ...
    const fetchNearby = async () => {
      setIsLoading(true);
      setCustomName('');
      const r = await searchNearby(clickedPoint.lat, clickedPoint.lon, searchRadius);
      setResults(r);
      setIsLoading(false);
    };
  ...
  const handleAddPoint = useCallback(
    (
      ...
      role: 'departure' | 'waypoint' | 'destination' | 'alt1' | 'alt2',
    ) => {
  ...
  const fplCoords = clickedPoint ? toFplCoords(clickedPoint.lat, clickedPoint.lon) : '';
    ...
    <AnimatePresence>
      {clickedPoint && (
        <motion.div
          ...
          className='absolute top-36 right-4 bottom-4 z-40 w-72 lg:w-96 hidden md:flex flex-col rounded-xl bg-white dark:bg-zinc-900 shadow-lg overflow-hidden select-none'
        >
          {/* Header */}
          <div className='flex items-center justify-between py-2 px-3 lg:py-3 lg:px-4 shrink-0 sticky left-0 top-0 bg-white dark:bg-zinc-900 z-50'>
            <div className='flex items-center gap-2'>
              <Crosshair className='h-4 w-4 lg:h-5 lg:w-5 text-sky-500' />
              <span className='font-semibold text-xs lg:text-sm text-zinc-700 dark:text-zinc-200 tracking-wider'>{t('selectedPoint')}</span>
            </div>
            <button
              onClick={onClose}
            ...
            >
              <X className='h-3.5 w-3.5 lg:h-4 lg:w-4 text-slate-500' />
            </button>
          </div>
          ...
          {/* Content */}
          <div className='flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3 bg-zinc-200 dark:bg-zinc-950'>
            {/* Clicked coords as custom point */}
            <div className='rounded-md bg-sky-500/10 p-2 lg:p-3'>
              <p className='text-xs lg:text-sm font-mono font-semibold text-sky-500 mb-1.5 lg:mb-2'>{fplCoords}</p>
              <input
                type='text'
                value={customName}
                onChange={e => setCustomName(e.target.value)}
                placeholder={t('customPointName')}
                className='w-full rounded-sm bg-white dark:bg-zinc-800 px-2.5 py-1.5 text-xs border border-slate-200 dark:border-zinc-700 focus:outline-none focus:ring-1 focus:ring-sky-500 mb-2 text-slate-800 dark:text-zinc-200 uppercase'
              />
            ...
            {/* Nearby results */}
            <h4 className='text-xs font-medium text-slate-500 dark:text-zinc-400 uppercase tracking-wider mt-1'>
            ...
            {!isLoading && results.length === 0 && <p className='text-xs text-slate-400 dark:text-zinc-500 text-center py-2 lg:py-3'>{t('noNearbyPoints')}</p>}
                ...
                return (
                  <div key={`${result.type}-${i}`} className='rounded-xl bg-slate-50 dark:bg-zinc-800/70 p-2.5'>
                    <div className='flex items-start gap-2 mb-2'>
                      ...
                      <div className='flex-1 min-w-0'>
                        <div className='flex items-center gap-1.5 flex-wrap'>
                          ...
                          <span
                            className={`text-[8px] lg:text-[9px] font-semibold uppercase px-1 py-0.5 lg:px-1.5 shrink-0 rounded-full ${
                              result.type === 'airport'
                    ...
                    <div className='grid grid-cols-2 gap-1.5'>
                      ...
                      <RoleButton icon={MapPin} label={t('asWaypoint')} onClick={() => handleAddPoint(result, 'waypoint')} className='col-span-2' />

                      {result.type === 'airport' ? (
                        <>
                          <RoleButton icon={Undo} label={t('asAlt1')} onClick={() => handleAddPoint(result, 'departure')} />

                          <RoleButton icon={Undo} label={t('asAlt2')} onClick={() => handleAddPoint(result, 'destination')} />
                        </>
                      ) : null}
                    </div>
                  </div>
...
function RoleButton({ icon: Icon, label, onClick, className }: { icon: React.ElementType; label: string; onClick: () => void; className?: string }) {
  return (
    <button
      onClick={onClick}
      className={`flex-1 flex items-center justify-center gap-1 rounded-sm bg-white dark:bg-zinc-700 px-2 py-1 text-xs lg:text-sm font-medium text-zinc-700 dark:text-zinc-200 hover:bg-sky-200 dark:hover:bg-sky-700 hover:text-sky-700 dark:hover:text-sky-200 transition-colors ${className}`}
      title={label}
    >
